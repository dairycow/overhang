...
function ProgressChart({ locationId, startDate, endDate }: ProgressChartProps) {
    ...
    return (
      <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600">log some sessions to see your progress over time!</p>
        </div>
      </div>
    )
  }
  ...
  // Create chart options with date filtering applied to x-axis
  const chartOptions = {
    ...lineChartOptions,
    ...
    scales: {
      ...
      x: {
        ...
        ...(startDate && { min: new Date(startDate).getTime() }),
        ...(endDate && { max: new Date(endDate).getTime() })
      }
    }
  }
